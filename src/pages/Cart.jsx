...
// Component trang Giỏ hàng (Cart Page)
const Cart = () => {
  // Lấy danh sách sản phẩm và các hàm thao tác từ CartContext
  const { cart, removeFromCart, updateQuantity } = useCart();
  
  // Tính tổng số tiền bằng cách cộng dồn tích (đơn giá * số lượng) của từng sản phẩm
  const totalPrice = cart.reduce((total, item) => total + (item.price * item.quantity), 0);

  // Trường hợp Giỏ hàng trống (cart.length === 0)
  if (cart.length === 0) {
    return (
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 bg-obsidian">
      <div className="max-w-6xl mx-auto px-6 lg:px-12">
        {/* Header giới thiệu số lượng sản phẩm */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Cột trái: Danh sách các sản phẩm đang có trong giỏ */}
          <div className="lg:col-span-2 space-y-4">
            {cart.map((item, index) => (
              <div 
                key={index} 
                className="glass-card rounded-2xl p-4 flex gap-4 animate-fade-in-up hover:border-gold/20 transition-all" 
                style={{ animationDelay: `${index * 0.05}s` }} // Delay tuần tự cho từng sản phẩm
              >
                {/* Ảnh sản phẩm */}
                ...
                {/* Thông tin & Bộ điều chỉnh số lượng */}
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <div className="flex items-center gap-0 rounded-xl overflow-hidden border border-white/10">
                      {/* Nút giảm số lượng (delta = -1) */}
                      <button className="w-8 h-8 flex items-center justify-center text-silver hover:text-ivory hover:bg-white/10 transition-all bg-transparent border-none text-sm cursor-pointer" onClick={() => updateQuantity(item.id, -1)} aria-label="Giảm">−</button>
                      
                      {/* Số lượng hiển thị hiện tại */}
                      <span className="w-8 h-8 flex items-center justify-center text-ivory text-sm font-medium bg-white/5">{item.quantity}</span>
                      
                      {/* Nút tăng số lượng (delta = +1) */}
                      <button className="w-8 h-8 flex items-center justify-center text-silver hover:text-ivory hover:bg-white/10 transition-all bg-transparent border-none text-sm cursor-pointer" onClick={() => updateQuantity(item.id, 1)} aria-label="Tăng">+</button>
                    </div>
          ...
          {/* Cột phải: Tóm tắt tổng quan đơn hàng & Thanh toán (Hỗ trợ sticky đứng yên khi cuộn trang) */}
          <div className="glass-card rounded-2xl p-6 h-fit lg:sticky lg:top-24">
            ...
            {/* Link chuyển sang trang Checkout */}
            <Link to="/checkout" className="block mt-6 no-underline">
              <button className="w-full py-3.5 rounded-xl bg-gradient-to-r from-gold to-gold-dark text-obsidian font-bold text-sm tracking-wider uppercase border-none hover:shadow-glow-gold transition-all flex items-center justify-center gap-2 cursor-pointer">
