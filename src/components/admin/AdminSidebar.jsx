      ...
      >
        ...
        <div className="p-3 border-t border-white/[0.06] shrink-0">
          <div className="flex items-center gap-3 px-3 py-2 mb-2">
            ...
            <div className="min-w-0">
              <p className="text-ivory text-xs font-medium truncate">{user?.name || 'Admin'}</p>
              <p className="text-silver-dark text-[10px] truncate">{user?.email || ''}</p>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-silver hover:text-rose hover:bg-rose/5 transition-all bg-transparent border-none"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
